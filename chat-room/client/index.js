const AppVue = new Vue({
  ...
  methods: {
    ...
    onOpen() {
      const { ws } = this;
      console.log(`websocket connection successful, readyState is: ${ws.readyState}`);
    ...
    onClone(error) {
      const { ws } = this;
      console.log(`socket has been closed, readyState is: ${ws.readyState}, error is :${error}`);
      ws.close();
    },
    onError(error) {
      const { ws } = this;
      console.log(`socket has been error, readyState is: ${ws.readyState}, error is :${error}`);
      // 连接失败之后，1s进行断线重连！
      setTimeout(() => {
        this.init();
      }, 1000);
